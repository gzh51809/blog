...
1. CLI，其全称是 command-line interface，也就是命令行界面。你我常用的git，算是比较著名的一个命令行工具了（你若是source tree党，请忽略这句话）：
...
而 vue-cli，首先，它是一个 CLI, 其次，它才是一个 Scafollding 工具，到 3.0 以后，它还算得上一个 Build 工具。
...
1. lovely-cli dev   -> 用开发环境的配置，用 webpack-dev-server 运行起一个webpack app.
2. lovely-cli build -> 用生成环境的配置，用 webpack 直接 build.
...
CLI 系列到此就结束了。接下来几个月，我会专注在 vue 和 react 的源码、对比和思考中，同时也会告诉你如何写一个vue或者react哦，所以下一个系列的标题，就暂定为《以更好的姿势看待 vue 和 react》吧，敬请关注。
